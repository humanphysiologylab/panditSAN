/*
   ...
   There are a total of 17 entries in the conductances.
 */
/*
...
void initFromFile(const char* filename, double* CONSTANTS, double* RATES, double* STATES, double* CONDUCTANCES)
{
  ...
  {
    getline(&line, &len, configfile);
    sscanf(line, "CONDUCTANCES[%d] = %f", &placeholder, &buffer);
    CONDUCTANCES[i] = buffer;
  }
  ...
  for(int i = 0; i < 17; i++)
  {
    fprintf(debug, "CONDUCTANCES[%d] = %f\n", i, CONDUCTANCES[i]);
  }
  fclose(debug);
...
void
*/
computeRates(double VOI, double* CONSTANTS, double* RATES, double* STATES, double* ALGEBRAIC, double* CONDUCTANCES)
{
...
ALGEBRAIC[29] = CONDUCTANCES[3]*CONSTANTS[14]*STATES[11]*( CONSTANTS[15]*STATES[12]+ CONSTANTS[16]*STATES[13])*(STATES[0] - ALGEBRAIC[28]);
ALGEBRAIC[30] = CONDUCTANCES[4]*CONSTANTS[18]*STATES[14]*STATES[15]*(STATES[0] - ALGEBRAIC[28]);
ALGEBRAIC[31] = CONDUCTANCES[5]*(((48.0000/(exp((STATES[0]+37.0000)/25.0000)+exp((STATES[0]+37.0000)/- 25.0000))+10.0000)*0.00100000)/(1.00000+exp((STATES[0] - (ALGEBRAIC[28]+76.7700))/- 17.0000))+( CONSTANTS[19]*(STATES[0] - (ALGEBRAIC[28]+1.73000)))/( (1.00000+exp(( 1.61300*CONSTANTS[2]*(STATES[0] - (ALGEBRAIC[28]+1.73000)))/( CONSTANTS[0]*CONSTANTS[1])))*(1.00000+exp((CONSTANTS[17] - 0.998800)/- 0.124000))));
ALGEBRAIC[39] = CONDUCTANCES[13]* (( (CONSTANTS[27]/(1.00000+ 0.124500*exp(( - 0.100000*STATES[0]*CONSTANTS[2])/( CONSTANTS[0]*CONSTANTS[1]))+ 0.0365000*CONSTANTS[68]*exp(( - STATES[0]*CONSTANTS[2])/( CONSTANTS[0]*CONSTANTS[1]))))*CONSTANTS[17])/(CONSTANTS[17]+CONSTANTS[28]))/(1.00000+pow(CONSTANTS[29]/STATES[1], 1.50000));
ALGEBRAIC[33] = CONDUCTANCES[7]* CONSTANTS[20]*STATES[16]*CONSTANTS[67]*(STATES[0] - ALGEBRAIC[28]);
ALGEBRAIC[37] =  CONDUCTANCES[11]*CONSTANTS[24]*(STATES[0] - ALGEBRAIC[28]);
...
ALGEBRAIC[26] =  CONDUCTANCES[1]*CONSTANTS[9]*pow(STATES[2], 3.00000)*STATES[3]*STATES[4]*(STATES[0] - ALGEBRAIC[25]);
ALGEBRAIC[27] = CONDUCTANCES[2]*CONSTANTS[11]*STATES[6]*( (0.900000+STATES[9]/10.0000)*STATES[7]+ (0.100000 - STATES[9]/10.0000)*STATES[8])*(STATES[0] - CONSTANTS[12]);
...
ALGEBRAIC[56] = CONDUCTANCES[16]*CONSTANTS[69]*STATES[26]*STATES[27]*(STATES[0] - CONSTANTS[12] + 106.5);
ALGEBRAIC[32] = CONDUCTANCES[6]*CONSTANTS[20]*STATES[16]*CONSTANTS[21]*(STATES[0] - ALGEBRAIC[25]);
ALGEBRAIC[34] = CONDUCTANCES[8]*( ALGEBRAIC[32]+ALGEBRAIC[33]);
ALGEBRAIC[35] = CONDUCTANCES[9]*CONSTANTS[22]*(STATES[0] - ALGEBRAIC[25]);
ALGEBRAIC[36] = CONDUCTANCES[10]*CONSTANTS[23]*(STATES[0] - CONSTANTS[25]);
ALGEBRAIC[38] = CONDUCTANCES[12]*(ALGEBRAIC[35]+ALGEBRAIC[36]+ALGEBRAIC[37]);
ALGEBRAIC[41] = CONDUCTANCES[15]*( CONSTANTS[31]*( pow(STATES[1], 3.00000)*CONSTANTS[26]*exp( 0.0374300*STATES[0]*CONSTANTS[33]) -  pow(CONSTANTS[10], 3.00000)*STATES[17]*exp( 0.0374300*STATES[0]*(CONSTANTS[33] - 1.00000))))/(1.00000+ CONSTANTS[32]*( STATES[17]*pow(CONSTANTS[10], 3.00000)+ CONSTANTS[26]*pow(STATES[1], 3.00000)));
ALGEBRAIC[40] = CONDUCTANCES[14]*( CONSTANTS[30]*STATES[17])/(STATES[17]+0.000400000);
ALGEBRAIC[13] = CONDUCTANCES[0]*(VOI>=CONSTANTS[4]&&VOI<=CONSTANTS[5]&&(VOI - CONSTANTS[4]) -  floor((VOI - CONSTANTS[4])/CONSTANTS[6])*CONSTANTS[6]<=CONSTANTS[7] ? CONSTANTS[8] : 0.00000);
...
void
computeVariables(double VOI, double* CONSTANTS, double* RATES, double* STATES, double* ALGEBRAIC, double* CONDUCTANCES)
{
ALGEBRAIC[6] = 1.00000/(1.00000+STATES[5]/0.0100000);
ALGEBRAIC[11] = 1.00000/(1.00000+exp((STATES[0]+87.5000)/10.3000));
ALGEBRAIC[0] = 1.00000/(1.00000+exp((STATES[0]+45.0000)/- 6.50000));
...
ALGEBRAIC[54] = 1.0/(1.0 + exp((36.0 - STATES[0])/6.1));
ALGEBRAIC[55] = 1.0/(1.0 + exp((66.0 + STATES[0])/6.0));
ALGEBRAIC[56] = CONDUCTANCES[16]*CONSTANTS[3]*CONSTANTS[69]*STATES[26]*STATES[27]*(STATES[0] - CONSTANTS[12] + 106.5);
ALGEBRAIC[57] = 0.6 + 5.4/(1.0 + exp(0.03*(STATES[0]+100.0)));
ALGEBRAIC[58] = 1.0 + 40.0/(1.0 + exp(0.08*(STATES[0]+65)));
}
...
int main(int argc, char *argv[]){
  ...
  clock_t t = clock();
  double CONSTANTS[70], RATES[28], STATES[28], ALGEBRAIC[59],CONDUCTANCES[17],VOI,start, end, step;
  //FILE *file;
  FILE *logfile;
  start = 0.0;
  end = 2.0;
  ...
  //initFromFile("default_ik1_0.txt", CONSTANTS, RATES, STATES);
  initFromFile(config_filename.c_str(), CONSTANTS, RATES, STATES, CONDUCTANCES);
  //file = fopen("test.csv","w");

  std::string file_csv_filename = (std::string) argv[2];
  std::ofstream file_csv;
  file_csv.open(file_csv_filename.c_str());

  file_csv << "T\tV\tI_na\tI_CaL\tI_CaT\tI_t\tI_ss\tI_f\tI_k1\tI_b\tI_NaK\tI_NCX\tI_CaP\tI_stim\tI_f_K\tI_f_Na\tI_b_Na\tI_b_Ca\tI_b_K\n";

  //fprintf(file,"T\tV\n");
  //fprintf(file, "T\tV\tI_na\tI_CaL\tI_CaT\tI_t\tI_ss\tI_f\tI_k1\tI_b\tI_NaK\tI_NCX\tI_CaP\tI_stim\tI_f_K\tI_f_Na\tI_b_Na\tI_b_Ca\tI_b_K\n");

  VOI = start;
  for (int i = 0; i < N; i++){
    VOI += step;
    //fprintf(file,"%f,",VOI);
    computeRates(VOI,CONSTANTS,RATES,STATES,ALGEBRAIC,CONDUCTANCES);
    ...
    if(i%100000 == 0)
    {
      ...
      file_csv << VOI << "\t" << STATES[0] << "\t" << ALGEBRAIC[26] << "\t" << ALGEBRAIC[27] << "\t" << ALGEBRAIC[56] << "\t" << ALGEBRAIC[29] << "\t" << ALGEBRAIC[30] << "\t" << ALGEBRAIC[34] << "\t" << ALGEBRAIC[31] << "\t" << ALGEBRAIC[38] << "\t" << ALGEBRAIC[39] << "\t" << ALGEBRAIC[41] << "\t" << ALGEBRAIC[40] << "\t" << ALGEBRAIC[13] << "\t" << ALGEBRAIC[33] << "\t" << ALGEBRAIC[32] << "\t" << ALGEBRAIC[35] << "\t" << ALGEBRAIC[36] << "\t" << ALGEBRAIC[37] << std::endl;
    }
    if(halt){
      printf("%f",VOI);
      break;
  ...
  computeVariables(VOI,CONSTANTS,RATES,STATES,ALGEBRAIC, CONDUCTANCES);
  //printf("ne ok %f,%f,%f\n ", RATES[0],STATES[0],ALGEBRAIC[5]);
  //fclose(file);
  file_csv.close();
  t = clock() - t;
  printf("%f, %f",CONDUCTANCES[0],CONDUCTANCES[2]);
  printf("time %ld",t);
